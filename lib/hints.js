...
export class HintMode {
  constructor(forceCapture, coordinateScale = 1) {
    this.active = false;
    this.buffer = '';
    this._resolve = null;
    this._client = null;
    ...
    this._coordinateScale = coordinateScale || 1;
  }
  ...
  async _selectHint(hint) {
    ...
    try {
      ...
      await dispatchClick(this._client, point.x, point.y, {
        coordinateScale: this._coordinateScale,
        source: `hint ${hint.label}`,
      });
    } catch (error) {
  ...
  updateCoordinateScale(scale) {
    this._coordinateScale = scale || 1;
  }
}
