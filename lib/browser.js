...
export async function startScreencast(client, { width, height, onFrame, format = 'png', quality = 85 }) {
  ...
  function onScreencastFrame({ data, sessionId }) {
    ...
    // Do not discard the change-detection frame. It gives immediate visual
    // feedback while the full-resolution capture is still rendering, which
    // can take seconds under software rendering or emulation.
    if (data) onFrame(data);
    capture();
  }
