...
export async function dispatchClick(client, x, y, { clickCount = 1, source = '?' } = {}) {
  ...
  const common = { x, y, button: 'left', clickCount };
  ...
  if (DEBUG_CLICK) {
    ...
    const line = JSON.stringify({
      t: new Date().toISOString(), source, css: [x, y], clickCount, before, clicked: after,
    });
    try {
