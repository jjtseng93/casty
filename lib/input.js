...
import { dispatchClick, getInputScale } from './click.js';
...
export function startInputHandling(client, cellWidth, cellHeight, zoom, bindings, pauseRender, forceCapture) {
  ...
  async function handleInput(str) {
    ...
    while ((match = mouseRe.exec(str)) !== null) {
      ...
      const scale = await getInputScale(client, _zoom);
      const eventX = x * scale;
      const eventY = y * scale;
      ...
      if (cb === MOUSE_SCROLL_UP) {
        ...
        if (!release) {
          ...
          lastClickX = x;
          lastClickY = y;
          pendingPress = { x, y, clickCount: lastClickCount };
        } else if (pendingPress) {
          const { x: px, y: py, clickCount } = pendingPress;
          pendingPress = null;
          await dispatchClick(client, px, py, { clickCount, coordinateScale: _zoom, source: `tap ${col},${row}` });
        } else {
        ...
        if (pendingPress) {
          ...
          const { x: px, y: py, clickCount } = pendingPress;
          pendingPress = null;
          await dispatchMouse(client, 'mousePressed', px * scale, py * scale, 'left', clickCount, 1);
        }
