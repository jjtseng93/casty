    ...
    while ((match = mouseRe.exec(str)) !== null) {
      ...
      if (cb === MOUSE_SCROLL_UP) {
        ...
        if (!release) {
          ...
          lastClickX = x;
          lastClickY = y;
          pendingPress = { x, y, col, row, clickCount: lastClickCount };
        } else if (pendingPress) {
          const { x: px, y: py, clickCount } = pendingPress;
          pendingPress = null;
          await dispatchClick(client, px, py, { clickCount, source: `tap ${col},${row}` });
        } else {
          await dispatchMouse(client, 'mouseReleased', x, y, 'left', lastClickCount || 1, 0);
        }
        ...
        if (pendingPress) {
          ...
          const { x: px, y: py, clickCount } = pendingPress;
          pendingPress = null;
          await dispatchMouse(client, 'mousePressed', px, py, 'left', clickCount, 1);
        }
        await dispatchMouse(client, 'mouseMoved', x, y, 'left', 0, 1);
